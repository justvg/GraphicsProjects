...
vec3 CalcDirLight(dir_light Light, vec3 Normal, vec3 ViewDir)
{
	...
	vec3 Diffuse = Light.Diffuse * DiffComponent * texture(Material.Diffuse, TexCoord);
	vec3 Specular = 0 * Light.Specular * SpecComponent * texture(Material.Specular, TexCoord);
...
vec3 CalcPointLight(point_light Light, vec3 Normal, vec3 ViewDir, vec3 FragmentPos)
{
	...
	vec3 Ambient = Light.Ambient * Atten * texture(Material.Diffuse, TexCoord);
	vec3 Diffuse = Light.Diffuse * Atten * DiffComponent * texture(Material.Diffuse, TexCoord);
	vec3 Specular = 0 * Light.Specular * Atten * SpecComponent * texture(Material.Specular, TexCoord);
